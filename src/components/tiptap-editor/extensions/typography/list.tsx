...
export const OrderedListExtension = OrderedList.extend({
  renderHTML({ HTMLAttributes }) {
    return [
      "ol",
      mergeAttributes(this.options.HTMLAttributes, HTMLAttributes, {
        class: "my-8 pl-6 md:pl-10 list-none is-ol [counter-reset:minimal-counter] space-y-4",
      }),
      0,
    ];
  },
  ...
  renderHTML({ HTMLAttributes }) {
    return [
      ...
      [
        "div",
        {
          class: "shrink-0 select-none flex items-center h-[1.625em] font-mono text-sm opacity-40 group-hover:opacity-100 transition-opacity",
        },
