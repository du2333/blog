...
import { createRunEffect } from "@/lib/effect/result";
import { handleImageRequest } from "@/lib/images/server";
import { createAppRuntime } from "@/services";
...
app.all("*", (c) => {
	// 为了向后兼容，仍然提供 db 和 auth（从 Runtime 中获取或直接创建）
	const db = getDb(c.env.DB);
	const auth = getAuth({ db, env: c.env });

	// 为每个请求创建 Runtime（因为 env 和 executionCtx 是请求特定的）
